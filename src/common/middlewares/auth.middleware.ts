...
export class AuthMiddleware implements NestMiddleware {
  ...
  async use(req: Request, res: Response, next: NextFunction) {
    ...
    if (refreshToken) {
      this.logger.debug(
        '[AUTH] Access Token expired/missing. Attempting refresh.'
      );
      const { payload: refreshPayload } =
        this.tokenUtil.verifyRefreshToken(refreshToken);

      if (refreshPayload && refreshPayload.uuid) {
        this.logger.debug('[AUTH] Valid Refresh Token. Rotating tokens.');

        // Remember Me 로직: 만료까지 8일 이상 남았으면 True로 간주 (Next.js 로직 포팅)
        ...
        const newCsrfToken = this.tokenUtil.generateCSRFToken();

        // 쿠키 재설정
        this.cookieUtil.setAuthCookies(
          res,
          {
        ...
        req.cookies[ACCESS_TOKEN_COOKIE] = newAccessToken;
        req.cookies[REFRESH_TOKEN_COOKIE] = newRefreshToken;

        return next();
      } else {
        this.logger.debug('[AUTH] Invalid Refresh Token.');
      }
    }
    ...
    if (accessToken || refreshToken) {
      this.logger.debug('[AUTH] Auth failed. Clearing cookies.');
      this.cookieUtil.clearAuthCookies(res);
    }
