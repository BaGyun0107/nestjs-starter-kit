...
export class AxiosService {
    ...
    this.axiosInstance.interceptors.response.use(
      ...
      (error) => {
        ...
        if (error.response) {
          ...
          const err: any = new Error(customError.message);
   ...
   *
   * @description
   * request 메서드는 axios를 사용하여 서버로 요청을 보내며,
   * 내부적으로 요청/응답 인터셉터, 에러 처리를 수행
   *
   * @example
   * const response = await axiosService.request('/api/endpoint', 'POST', { data, headers: {} });
   */
  async request(
    url: string,
    method: string = 'GET',
